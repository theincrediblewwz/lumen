...
| ADR-043 | 阅读器目录栏可拖动调宽：宽度进 `readerPrefs`（`tocWidth`，180–640px），把手做成 `.reader-main` 的同级 flex 项而非目录内的绝对定位子元素 | 用户报：目录条目文字太长只能省略号截断、只有悬停才看到全貌，而目录栏宽度写死 `flex: 0 0 232px` 没法调。决策分三部分：① **持久化**——`ReaderPrefs` 增加 `tocWidth`，配 `TOC_MIN/MAX/DEFAULT` 与 `clampToc()`（非有限值回退默认，防 localStorage 被改坏）；② **把手位置**——放在 `.reader-toc` 之后、`.reader-stage` 之前，作为 `.reader-main` 的**同级 flex 项**（`flex: 0 0 8px`，`::after` 画 2px 提示线）。不做成目录内的 `position:absolute` 子元素，是因为 `.reader-toc` 是 `overflow-y:auto` 的滚动容器，绝对定位子元素会随内容滚动、还会被横向裁掉；③ **交互**——pointer 事件 + `setPointerCapture`（指针移出把手/窗口仍跟手，鼠标·触控板·触摸通用），拖动中只更新本地 state 跟手、**松手才写 prefs**（避免每帧写 localStorage），键盘 `←/→` 微调（Shift 加速）、`Home`/双击复位，拖动时给根节点加 `.is-resizing` 关掉文字选中。双页模式下目录变宽会改变正文可用宽度，故把 `prefs.tocWidth` 并入重新分页的依赖（松手后重排一次，避免拖动中每帧重排）。 |
| ADR-044 | 导入文档的表格预处理：`guessMath` 必须放过分隔行，形近/不可见字符先归一化，超列单元格并回末格 | 用户报：导出的 md 是规范表格，软件里却「压根没渲染」。定位出**两个独立成因**：① **guessMath 吃掉分隔行**（主因）——导入文档默认 `guessMath:true`（settings.ts），而 `looksLikeMath` 的「绝对值/范数」规则 `/\|[^\|]+\|/` 会命中 GFM 分隔行 `\|---\|`，被包成 `$|---|---|$` 后整张表降级为段落（ADR-037 只在 AI 回答里绕开了 guessMath，导入文档仍中招）。② **形近/不可见字符**——PDF/网页复制常混入零宽空格 U+200B（「阶段」变「阶<ZWSP>段」）、形近竖线 U+2223/U+2502/U+FF5C（markdown-it 只认 ASCII `|`）、形近横线 U+2212/U+2013/U+2500（分隔行不合法）。决策：新增纯函数 `normalizeTablePipes()`，在 `renderMarkdown` 里**始终执行**（与 guessMath 无关），顺序在数学定界符归一化之后、猜测渲染之前；跳过代码围栏与行内代码；不可见字符与形近竖线全局替换，形近横线**只在分隔行里**替换（避免误伤正文的减号/破折号）；表头与分隔行之间夹空行时补掉。`preprocessGuessMath` 增加表格分支：**分隔行原样放过**，数据行按未转义 `|` 切分后**逐单元格猜测**（保住结构的同时单元格里的裸公式照样渲染）。另修一处静默丢内容：单元格内出现未转义竖线（数学范数 `|r[h]|`）时，多出来的单元格会被 markdown-it 直接丢弃导致整格空白——按表头列数把多余单元格**并回最后一格**，竖线转义成 `\|`（渲染出来仍是 `|`），且这类单元格不再交给 guessMath（否则 KaTeX 会拿到 `\|` 报错标红）。回归固化在 `src/reader/table.test.ts`（14 用例）。 |
| ADR-045 | Android 共同行协议与可恢复 WebDAV 同步 | 用户批准首版跨设备自动同步。每个 Lumen 白板映射 portable projects，桌面夹与未知字段原样保留。协议 core 精确 vendor 并记录 SHA-256；先落盘 outbox 再网络，冲突不静默覆盖，跨实体引用冲突保留投影。Rust 统一原子替换、before-image journal 与完整库 CAS；网络阶段不阻塞编辑。 |
| ADR-046 | 对话转文档/新节点共享幂等来源 | 对话单条、勾选多条、全部已完成消息均保存原文；共享键 chat:conversationId:orderedMessageIds，路径另作 SHA-256。文档、节点、可选父连线、原会话和来源记录同事务；保留更晚的磁盘聊天内容。预览构建独立配置和凭据命名空间，不触正式资料。 |
| ADR-047 | Android 预览独立于桌面正式发布 | 用户要求安卓版进入 Lumen 公开仓库并发布。仅导入已跟踪 Android 客户端源码，保留既有包名和本地数据身份；`android-v*` 标签避开桌面 `v*` 构建工作流。当前 APK 使用调试证书且真实三端/物理机验收未完成，所以标为 GitHub prerelease，附 SHA-256、签名和备份/升级限制；正式签名与跨证书迁移另立验收门槛。 |
